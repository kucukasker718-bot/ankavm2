...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('vmPanel', () => ({
        ...
        systemModules: [
            ...
            { id: 'backup', name: 'Yedekleme & Snapshot Otomasyonu', desc: 'Sistemi zamanlanmış görevlerle tam yedekler.', icon: 'fa-clock-rotate-left', active: true },
            { id: 'vlan', name: 'Gelişmiş Ağ İzolasyonu (VLAN)', desc: 'Müşteriler arası trafiği Layer-2 bazında izole eder.', icon: 'fa-network-wired', active: true },
            { id: 'loadbalancer', name: 'Yük Dengeleyici (HA)', desc: 'Sunucular arası trafik yükünü dengeler.', icon: 'fa-scale-balanced', active: true },
            { id: 'ddos', name: 'DDoS Koruma & Firewall', desc: 'Gelişmiş paket analizi ile port bazlı saldırıları engeller.', icon: 'fa-shield-virus', active: true },
            { id: 'whmcs', name: 'WHMCS Tam Entegrasyon', desc: 'WHMCS ile çift yönlü tam otomasyon.', icon: 'fa-plug', active: true },
                ...
                try {
                    ...
                    if (response && !response.error) {
                        ...
                        for (let file in this.activeDownloads) {
                            if (this.activeDownloads[file].status === 'downloading') {
                                allDone = false;
                                break;
                            }
                        }
                                            ...
                                            <tr class="hover:bg-slate-800/30 transition">
                                                <td class="p-4 flex items-center space-x-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <template x-if="activeDownloads[cImg.filename]">
                                                            <div class="mt-2 w-48 bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                                                <div class="bg-brand-500 h-1.5 transition-all duration-300" :style="`width: ${activeDownloads[cImg.filename].progress}%`"></div>
                                                            </div>
                                                            <div class="text-[9px] text-slate-400 mt-0.5 font-mono" x-text="`${activeDownloads[cImg.filename].progress}% - ${activeDownloads[cImg.filename].status}`"></div>
                                                        </template>
                                                    </div>
                                                </td>
